...
float update_block(int st_row, int end_row, int st_col, int end_col, float *old_wave_map, float *new_wave_map) {
    ...
    for (int row = st_row; row < end_row; row++) {
        for (int col = st_col; col < end_col; col++) {
            ...
            new_wave_map[row * MAP_WIDTH + col] = 0.2 * (old_wave_map[upper_row * MAP_WIDTH + col] +
                                                         old_wave_map[lower_row * MAP_WIDTH + col] +
                                                         old_wave_map[row * MAP_WIDTH + left_col] +
                                                         old_wave_map[row * MAP_WIDTH + right_col] +
                                                         old_wave_map[row * MAP_WIDTH + col]);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < ITER_NUM; i++) {
        ...
        if (rank == 0) {
            ...
            MPI_Recv(old_wave_map + (MAP_HEIGHT - 1) * MAP_WIDTH, MAP_WIDTH, MPI_FLOAT, size - 1, 0, MPI_COMM_WORLD,
                     nullptr);
            ...
            MPI_Recv(old_wave_map + (st_row - 1) * MAP_WIDTH, MAP_WIDTH, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD,
                     nullptr);
            ...
            MPI_Recv(old_wave_map + (end_row % MAP_HEIGHT) * MAP_WIDTH, MAP_WIDTH, MPI_FLOAT, (rank + 1) % size, 1,
