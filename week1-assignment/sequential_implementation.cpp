...
void shift_rows() {
    ...
    for (int row = 0; row < MESSAGE_BLOCK_SIZE; ++row) {
        for (int shifts = 0; shifts < row; ++shifts) {  // shift_row shifts array one position at one time, row decides how many times shift_row will be called.
            shift_row(row);
        }
    }
}
...
// TODO: m'[row, column] = c[r][3] m[3][column]^4 + c[r][2] m[2][column]^3 + c[r][1] m[1][column]^2 + c[r][0]m[0][column]^1
